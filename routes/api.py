...
def storage_stats():
    ...
    return jsonify({
        ...
        'type_breakdown': [
            {'type': t, 'count': c, 'size': s or 0} for t, c, s in type_counts
        ]
    })
...
def recent_files():
    ...
    return jsonify([{
        'id': f.id, 'name': f.original_filename, 'type': f.file_type,
        'size': f.size_human(), 'created_at': f.created_at.isoformat()
    } for f in files])
