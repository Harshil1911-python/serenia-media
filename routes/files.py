...
def log_activity(user_id, action, target_type=None, target_id=None, target_name=None):
    act = Activity(user_id=user_id, action=action, target_type=target_type,
                   target_id=target_id, target_name=target_name, ip_address=request.remote_addr)
    db.session.add(act)
...
def upload():
    if request.method == 'POST':
        ...
        for f in request.files.getlist('files'):
            ...
            db_file = File(filename=unique_name, original_filename=original,
...
def share_with_user(file_id):
    ...
    db.session.add(share)
    log_activity(current_user.id, 'share', 'file', file_id,
                 f'{f.original_filename} → {username}')
    db.session.commit()
    share_url = url_for('files.shared_file', token=share.token, _external=True)
